...
function csvToRows(text) {
  ...
  // Fast path. The NOAA export contains no quoted fields, and a plain split is
  // several times quicker than the character scanner over 36,000 rows.
  if (text.indexOf('"') === -1) {
...
function trendStats(points, iterations = SURROGATES) {
  ...
  for (let i = 0; i < n; i++) { xs[i] = good[i].year - x0; ys[i] = good[i].value; }

  // Deviations and Sxx depend only on the year positions, so they are computed
  // once and reused for every surrogate. A slope is then a single pass:
  // sum(dx * y) / Sxx, since the deviations already sum to zero.
  let xSum = 0;
  for (let i = 0; i < n; i++) xSum += xs[i];
  const xBar = xSum / n;
  const dx = new Float64Array(n);
  let sxx = 0;
  for (let i = 0; i < n; i++) { dx[i] = xs[i] - xBar; sxx += dx[i] * dx[i]; }

  const slopeOf = arr => {
    let s = 0;
    for (let i = 0; i < n; i++) s += dx[i] * arr[i];
    return s / sxx;
  };

  const slope = slopeOf(ys);
  let ySum = 0;
  for (let i = 0; i < n; i++) ySum += ys[i];
  const intercept = ySum / n - slope * xBar;
  ...
  // Box-Muller produces two independent draws per pair of uniforms; keeping the
  // second halves the transcendental calls in the inner loop.
  let spare = null;
  const draw = () => {
    if (spare !== null) { const held = spare; spare = null; return held; }
    let u = 0, v = 0;
    while (u === 0) u = rand();
    while (v === 0) v = rand();
  ...
  const stationarySd = phi < 1 ? sigma / Math.sqrt(1 - phi * phi) : sigma;
  const surrogate = new Float64Array(n);
  let atLeastAsLarge = 0;
  for (let k = 0; k < iterations; k++) {
    let prev = draw() * stationarySd;
    for (let i = 0; i < n; i++) {
      prev = phi * prev + sigma * draw();
      surrogate[i] = prev;
    }
    if (Math.abs(slopeOf(surrogate)) >= absSlope) atLeastAsLarge++;
  }
